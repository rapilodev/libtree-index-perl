package Tree::Simple:XS::Visitor;
...
sub visit {
	my ($self, $tree) = @_;
	(blessed($tree) && $tree->isa("Tree::Simple:XS"))
            || die "Insufficient Arguments : You must supply a valid Tree::Simple:XS object";
...
Tree::Simple:XS::Visitor - Visitor object for Tree::Simple:XS objects

=head1 SYNOPSIS

  use Tree::Simple:XS;
  use Tree::Simple:XS::Visitor;

  # create a visitor instance
  my $visitor = Tree::Simple:XS::Visitor->new();

  # create a tree to visit
  my $tree = Tree::Simple:XS->new(Tree::Simple:XS->ROOT)
                         ->addChildren(
                             Tree::Simple:XS->new("1.0"),
                             Tree::Simple:XS->new("2.0")
                                         ->addChild(
                                             Tree::Simple:XS->new("2.1.0")
                                             ),
                             Tree::Simple:XS->new("3.0")
                             );
...
for the visitors found in the B<Tree::Simple:XS::VisitorFactory> distribution, which includes a
...
to the old way of using it. So the original example code still works:
  ...
  my @accumulator;
  my $visitor = Tree::Simple:XS::Visitor->new(sub {
                        my ($tree) = @_;
                        push @accumulator, $tree->getNodeValue();
                        },
                        Tree::Simple:XS::Visitor->RECURSIVE);
...
This object is still pretty much a wrapper around the Tree::Simple:XS C<traverse> method,
...
B<Tree::Simple:XS> object as its only argument. The second argument is optional, it can be
used to set the depth to which the function is applied. If no depth is set, the function
is applied to the current B<Tree::Simple:XS> instance. If C<$depth> is set to C<CHILDREN_ONLY>,
then the function will be applied to the current B<Tree::Simple:XS> instance and all its
immediate children. If C<$depth> is set to C<RECURSIVE>, then the function will be applied
to the current B<Tree::Simple:XS> instance and all its immediate children, and all of their
children recursively on down the tree. If no C<$depth> is passed to the constructor,
then the function will only be applied to the current B<Tree::Simple:XS> object and none
 of its children.
...
accept a single argument, which is the current Tree::Simple:XS object.
...
The C<visit> method accepts a B<Tree::Simple:XS> and applies the function set in C<new> or
...
B<Tree::Simple:XS>) and is based on an (non-publicly released) module which I had used in production
...
B<Tree::Simple:XS::VisitorFactory>.
