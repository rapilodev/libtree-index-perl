...
sub new {
    ...
    if (defined $parent_obj) {
        die "Insufficient Arguments : " unless ref($parent_obj) eq 'Tree::Simple:XS';
...
sub addChild {
    ...
    die "Insufficient Arguments : Child must be a Tree::Simple:XS object"
        unless (defined $child_obj && ref($child_obj) eq 'Tree::Simple:XS');
...
sub _setParent {
    ...
    if (!defined $parent_obj || !ref($parent_obj) || !eval { $parent_obj->isa('Tree::Simple:XS') }) {
