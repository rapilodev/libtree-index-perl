...
my $SUB_TREE = Tree::Simple:XS->new("sub tree test");
my $MISC_OBJECT = bless({}, "Misc");

$tree->addChildren(
		Tree::Simple:XS->new("non-ref"),	
		Tree::Simple:XS->new($SCALAR_REF),	
		Tree::Simple:XS->new($ARRAY_REF),
		Tree::Simple:XS->new($HASH_REF),
		Tree::Simple:XS->new($CODE_REF),
		Tree::Simple:XS->new($REGEX_REF),
		Tree::Simple:XS->new($MISC_OBJECT),
		Tree::Simple:XS->new($SUB_TREE),
		Tree::Simple:XS->new($REF_TO_REF)        
		);

my $clone = $tree->clone();
isa_ok($clone, 'Tree::Simple:XS');

# make sure all the parentage is correct
is($clone->getParent(), Tree::Simple:XS->ROOT, '... the clones parent is a root');
...
# they should both be Tree::Simple:XS objects
is(ref($clone->getChild(7)->getNodeValue()), "Tree::Simple:XS", '... these should be Tree::Simple:XS');
is(ref($tree->getChild(7)->getNodeValue()), "Tree::Simple:XS", '... these should be Tree::Simple:XS');
# but different ones
isnt($clone->getChild(7)->getNodeValue(), $tree->getChild(7)->getNodeValue(), 
	'... these should be different Tree::Simple:XS objects');	
# with the same value	
