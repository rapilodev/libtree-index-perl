...
BEGIN {
    use_ok('Tree::Adapter::Simple') or BAIL_OUT("Could not load Tree::Adapter::Simple package");
}
...
my $root = Tree::Adapter::Simple->new("Root Anchor", Tree::Adapter::Simple->ROOT);
isa_ok($root, 'Tree::Adapter::Simple', "Root node initialization");
...
if ($has_devel_size) {
    no warnings 'once';
    my $v_size  = total_size(\@Tree::Adapter::Simple::NODE_VALUES);
    my $p_size  = total_size(\@Tree::Adapter::Simple::PARENTS);
    my $c_size  = total_size(\@Tree::Adapter::Simple::CHILDREN);
    my $u_size  = total_size(\@Tree::Adapter::Simple::UIDS);
...
print "-----------------------------------------------------------------------\n";
#use Data::Dumper;
#print Dumper(\@Tree::Adapter::Simple::CHILDREN);
