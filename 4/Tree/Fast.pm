...
sub getParent {
    my ($self) = @_;
    die "Cannot query parent on an unattached node" unless defined $self->[0];
...
sub children {
    my ($self) = @_;
    die "Cannot query children on an unattached node" unless defined $self->[0];
    my @out;
    my $tree     = $self->[0];
...
sub new {
    ...
    if (blessed($value) && $value->isa('Tree::Node'))
    {
        ...
        $self->_insert_child_data(undef,
            Tree::Node::Data->new($value));
    }
    return $self;
}
...
sub _insert_child_data {
    my ($self, $parent_id, $data_node) = @_;
    my $new_id = $self->{next_id}++;
    $self->{nodes}[$new_id] = $data_node;
    return $new_id unless defined $parent_id;    # Root short-circuit
