package Tree::Adapter::Simple {
    ...
    my @basic_fields =
      qw(parent first_child last_child prev_sibling next_sibling);
    ...
    sub new {
        ...
        unless ($instantiated++) {
            Tree::set_custom_fields(@custom_fields);
            #Tree::Node::set_custom_fields(@custom_fields);
        }
        my ($tree, $node);
        if (!defined $parent) {
            $tree = Tree->new();
            ...
            $tree = Tree->new();
            $node = $tree->root;
        } elsif (ref($parent) && $parent->isa('Tree::Adapter::Simple')) {
            $tree = $parent->_internal_tree;
            $node = $parent->_internal_node->add_child();
        } else {
            die
"Insufficient Arguments : parent argument must be a Tree::Adapter::Simple object";
        }
    ...
    sub addChild {
        my ($self, $child) = @_;
        croak "Child must be a Tree::Adapter::Simple object"
    ...
    sub getAllChildren {
        my ($self) = @_;
        my @children = 
            map {$self->_new_treenode($self, $self->{_tree}, $_)}
            $self->{_node}->children;
    ...
    sub removeChildAt {
        ...
        $self->{_tree}->_remove_node($target_node);
    ...
    sub getHeight {
        ...
        my $max = 0;
        for my $child ($self->getAllChildren) {
            my $heigth = $child->getHeight;
            $max = $heigth if $heigth > $max;
        }
        return 1 + $max;
    }
