...
my $SUB_TREE = Tree::Fast->new("sub tree test");
my $MISC_OBJECT = bless({}, "Misc");

$tree->addChildren(
		Tree::Fast->new("non-ref"),	
		Tree::Fast->new($SCALAR_REF),	
		Tree::Fast->new($ARRAY_REF),
		Tree::Fast->new($HASH_REF),
		Tree::Fast->new($CODE_REF),
		Tree::Fast->new($REGEX_REF),
		Tree::Fast->new($MISC_OBJECT),
		Tree::Fast->new($SUB_TREE),
		Tree::Fast->new($REF_TO_REF)        
		);

my $clone = $tree->clone();
isa_ok($clone, 'Tree::Fast');

# make sure all the parentage is correct
is($clone->getParent(), Tree::Fast->ROOT, '... the clones parent is a root');
...
# they should both be Tree::Fast objects
is(ref($clone->getChild(7)->getNodeValue()), "Tree::Fast", '... these should be Tree::Fast');
is(ref($tree->getChild(7)->getNodeValue()), "Tree::Fast", '... these should be Tree::Fast');
# but different ones
isnt($clone->getChild(7)->getNodeValue(), $tree->getChild(7)->getNodeValue(), 
	'... these should be different Tree::Fast objects');	
# with the same value	
